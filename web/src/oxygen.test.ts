...
describe("conditions with a CMRO₂ change", () => {
  const c: Condition = { label: "act", perturbations: [{ name: "scale_diameter", params: { layers: [3], factor: 1.2 } }] };
  it("adds CMRO₂ on the same layers and removes it again", () => {
  ...
  it("keeps the CMRO₂ region in step with the diameter region", () => {
    const withM = buildCondition(c, conditionParts(c).diameter, 1.1);
...
describe("oxygen colour modes", () => {
  ...
  it("offers PO₂, SO₂ and SO₂ change when a run has oxygen", () => {
    const labels = colorByOptions(run, graph).map((o) => o.label);
    expect(labels).toContain("PO₂: baseline");
    expect(labels).toContain("SO₂ change: act");
  });
  it("shows SO₂ change on the diverging scale", () => {
